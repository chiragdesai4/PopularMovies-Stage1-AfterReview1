...
/**
 * Created by Chirag Desai on 17-03-2017.
 */

public class FetchMovieAsyncTask extends AsyncTask<String, Void, Moviename[]> {
    ...
    @Override
    protected Moviename[] doInBackground(String... params) {
    ...
    private Moviename[] getMoviesDataFromJson(String moviesJsonStr) throws JSONException {
        // JSON tags
        ...
        Moviename[] movies = new Moviename[resultsArray.length()];

        // Traverse through movies one by one and get data
        for (int i = 0; i < resultsArray.length(); i++) {
            // Initialize each object before it can be used
            movies[i] = new Moviename();
    ...
    @Override
    protected void onPostExecute(Moviename[] movies) {
